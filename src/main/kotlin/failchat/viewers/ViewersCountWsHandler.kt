...
class ViewersCountWsHandler(private val config: Configuration) : WsMessageHandler {
    override val expectedType = InboundWsMessage.Type.VIEWERS_COUNT

    @Volatile var viewersCounter: ViewersCounter? = null
    ...
    override fun handle(message: InboundWsMessage) {
        ...
        val enabledOrigins =
            COUNTABLE_ORIGINS.filter { config.getBoolean("${it.commonName}.enabled") }

        val messageNode =
            nodeFactory.objectNode().apply {
                put("type", "viewers-count")
                putObject("content").apply { enabledOrigins.forEach { putNull(it.commonName) } }
            }
