...
class BttvApiClient(
    ...
    fun loadGlobalEmoticons(): CompletableFuture<List<BttvEmoticon>> {
        ...
        val request = Request.Builder().url(globalEmoticonsUrl).get().build()

        return httpClient.newCall(request).toFuture().thenUse {
    ...
    fun loadChannelEmoticons(channel: String): CompletableFuture<List<BttvEmoticon>> {
        ...
        return httpClient.newCall(request).toFuture().thenUse {
            when (it.code) {
                ...
                404 -> {
                    throw BttvChannelNotFoundException(channel)
                }

                else -> {
                    throw UnexpectedResponseCodeException(it.code)
                }
            }
            val responseBody = it.body ?: throw UnexpectedResponseException("null body")
            val bodyString = responseBody.string()
    ...
    private fun parseEmoticons(responseBody: String, origin: Origin): List<BttvEmoticon> {
        ...
        return channelEmoticonsNode.get("emotes").map {
            BttvEmoticon(
                origin,
                it.get("code").asText(),
