...
class SevenTvApiClient(
    ...
    suspend fun loadGlobalEmoticons(): List<SevenTvEmoticon> {
        val request = Request.Builder().url("$apiUrl/emote-sets/$globalEmoteSetId").get().build()

        return httpClient.newCall(request).await().use {
            ...
            val emoteSet =
                objectMapper.readValue<SevenTvEmoteSetResponse>(responseBody.byteStream())
    ...
    suspend fun loadChannelEmoticons(channelId: Long): List<SevenTvEmoticon> {
        val request = Request.Builder().url("$apiUrl/users/twitch/$channelId").get().build()
        ...
        return httpClient.newCall(request).await().use {
            ...
            val responseBody = it.body ?: throw UnexpectedResponseException("null body")
            val channelResponse =
                objectMapper.readValue<SevenTvChannelResponse>(responseBody.byteStream())
            parseEmoteSet(channelResponse.emoteSet, Origin.SEVEN_TV_CHANNEL)
        }
    }

    private fun parseEmoteSet(
        emoteSet: SevenTvEmoteSetResponse,
        origin: Origin,
    ): List<SevenTvEmoticon> {
        return emoteSet.emotes.mapNotNull { emote ->
            // 7tv provides emotes in webp format, but JavaFX WebView doesn't support it
            val gifEmotes =
                emote.data.host.files.filter { it.format == "GIF" }.sortedBy { it.width }
            val pngEmotes =
                emote.data.host.files.filter { it.format == "PNG" }.sortedBy { it.width }
