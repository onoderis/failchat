...
fun ScheduledExecutorService.schedule(delay: Duration, command: () -> Unit): ScheduledFuture<*> =
    this.schedule(command, delay.toMillis(), TimeUnit.MILLISECONDS)

fun ScheduledExecutorService.scheduleWithCatch(
    delay: Duration,
    command: () -> Unit,
): ScheduledFuture<*> =
    schedule(delay) {
...
fun ExecutorService.executeWithCatch(task: () -> Unit) = execute {
    try {
        task.invoke()
    } catch (t: Throwable) {
