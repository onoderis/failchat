...
class EmojiHandler : MessageHandler<ChatMessage> {
    override fun handleMessage(message: ChatMessage) {
        val transformedText =
            EmojiParser.parseFromUnicode(message.text) {
                val hexEmoji = toHex(it.emoji.unicode)
                val hexFitzpatrick: String? = it.fitzpatrick?.let { f -> toHex(f.unicode) }

                val emojiHexSequence =
                    if (hexFitzpatrick == null) {
                        hexEmoji
                    } else {
                ...
                val emoticonUrl =
                    "https://cdnjs.cloudflare.com/ajax/libs/twemoji/13.0.1/svg/$emojiHexSequence.svg"
