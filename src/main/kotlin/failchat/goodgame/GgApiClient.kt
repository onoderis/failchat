...
class GgApiClient(
    ...
    private companion object {
        ...
        val channelEmoticonsPattern: Pattern =
            Pattern.compile("""Channel_Smiles ?: ?(\{.+?\}\]\}),""")
    }

    private val apiUrl = apiUrl.withSuffix("/")

    suspend fun requestEmoticonList(): List<GgEmoticon> {
        val request = Request.Builder().url(emoticonsJsUrl).get().build()
    ...
    suspend fun requestChannelId(channelName: String): Long {
        ...
        return requestChannelStatus(channelName).first().get("stream_id").longValue()
    }
    ...
    private suspend fun requestChannelStatus(channelName: String): JsonNode {
        ...
        val request = Request.Builder().get().url(apiUrl + "getchannelstatus" + parameters).build()
