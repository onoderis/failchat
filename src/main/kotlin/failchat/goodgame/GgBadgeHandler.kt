...
class GgBadgeHandler(private val channel: GgChannel, config: Configuration) :
    ...
    private companion object {
        ...
        const val starBadge = "&#59730;"
        val badgeNameToCharEntity: Map<String, String> =
            mapOf(
                "coin" to "&#58892;",
                "eagle" to "&#58897;",
                "cup" to "&#58895;",
        ...
        val sponsorLevelToSponsorBadge: Map<Int, String> =
            mapOf(
                1 to "&#58892;",
                2 to "&#58897;",
                3 to "&#58895;",
        ...
        val subDurationToStarBadge: Map<Int, String> =
            mapOf(
                1 to "&#59730;",
                2 to "&#59729;",
                3 to "&#59728;",
    ...
    override fun handleMessage(message: GgMessage) {
        ...
        when (badgeName) {
            "star" -> {
                ...
                val premiumType =
                    when {
                        message.subscriptionDuration.containsKey(channel.id) -> {
                            Premium.CHANNEL
                        }

                        message.subscriptionDuration.containsKey(0) -> {
                            Premium.GLOBAL
                        }

                        else -> {
                            logger.warn(
                                "GG user '{}' have 'star' icon, but he is not channel subscriber or global subscriber",
                                message.author.name,
                            )
                            return
                        }
                ...
                val badge: Badge =
                    when (premiumType) {
                        Premium.CHANNEL -> {
                            ImageBadge("$badgeUrl/${channel.id}-$duration-16.png", RASTER)
                        }

                        Premium.GLOBAL -> {
                            val badgeEntity =
                                subDurationToStarBadge.get(duration)
                                    ?: run {
                                        logger.warn(
                                            "Unknown global premium badge. user: '{}', duration: {}",
                                            message.author.name,
                                            duration,
                                        )
                                        return
                            ...
                            // global subscriber with color 'premium-personal' actually have
                            // 'premium' color
                            val realColorName =
                                message.authorColorName.let {
                                    if (it == "premium-personal") {
                                        "premium"
                                    } else {
                                        it
                                    }
            ...
            "" -> {
                ...
                if (message.authorRights == 10) {
                    message.addBadge(
                        CharacterBadge(
                            helperBadgeEntity,
                            GgColors.byRole.get(message.authorColorName)
                                ?: findColorOrDefault("streamer-helper"),
                        )
                    )
                }
            }
                ...
                val badgeCharacterEntity =
                    badgeNameToCharEntity.get(badgeName)
                        ?: sponsorLevelToSponsorBadge.get(
                            message.sponsorLevel
                        ) // reproduce GG behaviour
                        ?: return

                message.addBadge(
                    CharacterBadge(
                        badgeCharacterEntity,
                        findColorOrDefault(message.authorColorName),
                    )
                )
            }
        }
    ...
    private fun findColorOrDefault(ggColorName: String): Color =
        GgColors.byRole.get(ggColorName) ?: GgColors.defaultColor
