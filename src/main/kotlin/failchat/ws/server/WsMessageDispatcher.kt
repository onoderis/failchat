...
class WsMessageDispatcher(
    ...
    private val handlers: Map<InboundWsMessage.Type, WsMessageHandler> =
        handlers
    ...
    suspend fun handleWebSocket(session: DefaultWebSocketServerSession) {
        session.incoming.consumeEach { frame ->
            ...
            val type =
                InboundWsMessage.Type.from(typeString)
                    ?: run {
                        logger.warn("Message received with unknown type '{}'", typeString)
                        return@consumeEach
                    }

            handlers[type]?.handle(InboundWsMessage(type, parsedMessage.get("content"), session))
                ?: logger.warn(
                    "Message consumer not found for a message with a type '{}'. Message: {}",
                    type,
                    frameText,
                )
        }
