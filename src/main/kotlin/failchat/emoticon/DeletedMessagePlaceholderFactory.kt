...
class DeletedMessagePlaceholderFactory(
    ...
    fun create(): DeletedMessagePlaceholder {
        ...
        val escapedText =
            text.let { Elements.escapeBraces(it) }.let { Elements.escapeLabelCharacters(it) }

        val processedText =
            SemicolonCodeProcessor.process(escapedText) { code ->
                val prefixAndCode = code.split("-", ignoreCase = true, limit = 2)
                val origin = prefixes[prefixAndCode.first()] ?: return@process ReplaceDecision.Skip

                val emoticon =
                    emoticonFinder.findByCode(origin, prefixAndCode[1])
