...
class EmoticonManager(private val storage: EmoticonStorage) {
    ...
    private fun <T : Emoticon> actualizeEmoticons(loadConfiguration: EmoticonLoadConfiguration<T>) {
        ...
        when (loadResult) {
            is LoadResult.Failure -> {
                logger.warn {
                    "Failed to load emoticon list for $origin. Outdated list will be used, count: $emoticonsInStorage"
                }
            }

            is LoadResult.Success -> {
                logger.info {
                    "Emoticon list loaded for $origin, count: ${loadResult.emoticonsLoaded}"
                }
            }
        }
    }

    private fun <T : Emoticon> loadEmoticons(
        loadConfiguration: EmoticonLoadConfiguration<T>
    ): LoadResult {
        val origin = loadConfiguration.origin
        ...
        val emoticons =
            try {
                ...
                logger.warn(e) {
                    "Failed to load emoticon list for $origin via bulk loader ${loadConfiguration.loader}"
                }
                return LoadResult.Failure
            }

        // Put data in storage
        val emoticonAndIdMapping =
            emoticons.map { EmoticonAndId(it, loadConfiguration.idExtractor.extractId(it)) }
