...
class EmoticonStorage : EmoticonFinder {
    ...
    private var originStorages: Map<Origin, OriginEmoticonStorage> =
        Origin.values
    ...
    override fun findByCode(origin: Origin, code: String): Emoticon? =
        originStorages.get(origin)!!.findByCode(code)

    override fun findById(origin: Origin, id: String): Emoticon? =
        originStorages.get(origin)!!.findById(id)

    override fun getAll(origin: Origin): Collection<Emoticon> =
        originStorages.get(origin)!!.getAll()

    fun getCount(origin: Origin): Int = originStorages.get(origin)!!.count()
