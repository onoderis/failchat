...
class YoutubeHtmlParser(private val objectMapper: ObjectMapper) {
    private companion object {
        val configStatementPattern = Pattern.compile("""ytcfg\.set\((\{.+\})\);""")!!
        val youtubeInitialDataPattern =
    ...
    fun extractInnertubeApiKey(youtubeConfig: JsonNode): String =
        youtubeConfig.get("INNERTUBE_API_KEY")?.textValue()
    ...
    fun extractInitialContinuation(ytInitialData: JsonNode): String {
        val continuation =
            ytInitialData.get("contents")?.get("liveChatRenderer")?.get("continuations")?.get(0)

        val continuationData =
            continuation?.get("invalidationContinuationData")
                ?: continuation?.get("timedContinuationData")

        return continuationData?.get("continuation")?.textValue()
