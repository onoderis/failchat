...
data class LiveChatResponse(val continuationContents: ContinuationContents) {
    data class ContinuationContents(val liveChatContinuation: LiveChatContinuation)
    ...
    data class ContinuationData(val timeoutMs: Int, val continuation: String)
    ...
    data class AddChatItemAction(val item: Item)

    data class Item(
        val liveChatTextMessageRenderer: LiveChatTextMessageRenderer? // todo is nullable?
    )

    data class AuthorBadge(val liveChatAuthorBadgeRenderer: LiveChatAuthorBadgeRenderer)
    ...
    data class CustomThumbnail(val thumbnails: List<Thumbnail>)
    ...
    data class Message(val runs: List<MessageRun>)

    data class MessageRun(val text: String?, val emoji: Emoji?)

    data class AuthorName(val simpleText: String)
    ...
    data class EmojiThumbnail(val url: String)

    data class EmojiAccessibility(val accessibilityData: EmojiAccessibilityData)

    data class EmojiAccessibilityData(val label: String)

    data class MarkChatItemsByAuthorAsDeletedAction(val externalChannelId: String)
}
