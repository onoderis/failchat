...
class TwitchRetriesTest {
    ...
    @Test
    fun shouldGetAndSaveNewTokenIfTokenDoesntExist() =
        runBlocking<Unit> {
            // Given
            ...
            // When
            val actual =
                doWithRetryOnAuthError(apiClientMock, secret, tokenContainerMock) { expected }
    ...
    @Test
    fun shouldNotRetryOnValidToken() =
        runBlocking<Unit> {
            // Given
            every { tokenContainerMock.getToken() } returns token
            var calls = 0
