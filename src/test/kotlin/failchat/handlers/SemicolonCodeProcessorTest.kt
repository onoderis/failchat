...
class SemicolonCodeProcessorTest {
    ...
    fun emptyString() {
        val initialString = ""
        val processedString =
            SemicolonCodeProcessor.process(initialString) { error("shouldn't be invoked") }
    ...
    fun replaceSingleCode() {
        val processedString =
            SemicolonCodeProcessor.process(":code:") {
    ...
    fun replaceSingleCodeAroundText() {
        val processedString =
            SemicolonCodeProcessor.process("text1 :code: text2") {
                assertEquals("code", it)
                ReplaceDecision.Replace("42")
            }
    ...
    fun skipSingleCode() {
        ...
        val processedString =
            SemicolonCodeProcessor.process(initialString) {
                assertEquals("code", it)
    ...
    fun replaceMultipleCodes() {
        ...
        val processedString =
            SemicolonCodeProcessor.process("hello :one: :two: :)") {
                codes.add(it)
    ...
    fun semicolonHell() {
        ...
        val processedString =
            SemicolonCodeProcessor.process(":w:wrong:right:w:w:") {
                codes.add(it)
